...
DEFAULT_SPARK_EC2_GITHUB_REPO = "https://github.com/jinliangwei/spark-ec2-setup"
...
def parse_args():
	...
	help="A directory to copy into / on the first main. " +
	     ...
	     "(default: %default).")
    parser.add_option(
	"--hadoop-major-version", default="2.7.3",
	help="Major version of Hadoop. Valid options only include 2.7.3. We only support 2.7.3 right now, this option is merely a place-holder " +
	     "(default: %default)")
    parser.add_option(
...
def validate_spark_version(version, repo):
...
def launch_cluster(conn, opts, cluster_name):
	...
	print("Requesting 1 main as spot instances with price $%.3f" %
	      (opts.spot_price,))
	...
	try:
		...
		if main_req_id in id_to_req and id_to_req[main_req_id].state == "active":
		    main_active = True
		if len(active_instance_ids) == opts.subordinates and main_active:
		    print("All %d spot instances granted" % (opts.subordinates + 1))
		    reservations = conn.get_all_reservations(active_instance_ids)
		    subordinate_nodes = []
		    for r in reservations:
			subordinate_nodes += r.instances
		    main_nodes = []
		    main_reservations = conn.get_all_reservations([id_to_req[main_req_id].instance_id])
		    main_nodes += main_reservations[0].instances
		    break
		else:
		    print("%d of %d subordinate spot instances granted, waiting longer" % (
			len(active_instance_ids) + 1 if main_active else len(active_instance_ids),
			opts.subordinates + 1))
	except:
	...
	print ("ERROR: --spot-price was not set; should always launch subordinates as spot instances, exit")
	sys.exit(0)
...
def setup_cluster(conn, main_nodes, subordinate_nodes, opts, deploy_ssh_key, format_ebs):
    main = get_dns_name(main_nodes[0], opts.private_ips)
    if deploy_ssh_key:
	permit_root_ssh_login(main, opts)
	...
	for subordinate in subordinate_nodes:
	    ...
	    permit_root_ssh_login(subordinate_address, opts)
    ...
    setup_spark_cluster(main, opts, format_ebs)
    print("Done!")


def setup_spark_cluster(main, opts, format_ebs):
    ssh(main, opts, "chmod u+x spark-ec2-setup/setup.sh")
    ssh(main, opts, "spark-ec2-setup/setup.sh " + format_ebs)
    print("Spark cluster started at http://%s:7077" % main)
